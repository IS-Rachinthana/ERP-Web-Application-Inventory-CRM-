...
export async function createProduct(f: FormData) { try { await prisma.product.create({ data: { sku: value(f,"sku"), name: value(f,"name"), description: value(f,"description") || null, costPrice: numeric(f,"costPrice"), sellingPrice: numeric(f,"sellingPrice"), reorderLevel: Math.max(0, Math.floor(numeric(f,"reorderLevel"))) } }); complete("Product added."); } catch(e) { fail(e); } }
